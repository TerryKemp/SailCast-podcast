...
function podcastsIndex (req, res, next) {
  Podcast
    .find()
    .then((podcasts) => res.render('podcasts/index', {
      podcasts
    }))
    .catch(next)
}
...
function podcastsUpdate (req, res, next) {
  Podcast
    ...
    .then(podcast => {
      ...
      return res.render('podcasts/show', {
        podcast
      })
    })
    .catch(next)
...
function getPodcasts (req, res) {
  rp(`https://itunes.apple.com/search?term=${req.params.query}&media=podcast&country=gb&limit=50`)
    .then(response => {
